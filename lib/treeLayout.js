...
// Scatters every node to a random position within an area sized to the node
// count, so a large tree doesn't end up impossibly cramped nor a small one
// lost in a huge empty canvas. Deliberately no overlap-avoidance or
// structure awareness - it's meant as a raw random starting point, with
// Auto Arrange/Auto Align Grid there to tidy up afterward.
export function shuffleLayout(currentNodes) {
  if (currentNodes.length === 0) return currentNodes;

  const side = Math.ceil(Math.sqrt(currentNodes.length));
  const areaWidth = side * 300;
  const areaHeight = side * 220;
